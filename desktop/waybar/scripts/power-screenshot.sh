...
# 菜单程序：优先使用当前已配置主题的 Rofi，Fuzzel 作为原生 Wayland 回退。
if command -v rofi >/dev/null 2>&1; then
    MENU_CMD='rofi -dmenu'
    MENU_PROMPT_FLAG='-p'
elif command -v fuzzel >/dev/null 2>&1; then
    MENU_CMD='fuzzel --dmenu'
    MENU_PROMPT_FLAG='--prompt'
else
    notify-send -u critical 'Screenshot' 'No supported menu launcher found (rofi or fuzzel).'
    exit 1
fi
...
menu_prompt() {
    ...
    printf '%s\n' "$@" | eval "$MENU_CMD $MENU_PROMPT_FLAG \"$esc_prompt\"" 2>/dev/null || true
}
