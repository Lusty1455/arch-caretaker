...
_pick_menu_backend() {
  ...
  for b in rofi fuzzel wofi bemenu fzf; do has "$b" && { echo "$b"; return; }; done
...
menu_rec() {
  if is_running; then
    echo "$(msg already_running)"
    emit_waybar_signal
    return 0
  fi

  local configured_mode="$RECORD_MODE"
  RECORD_MODE="ask"
  start_rec
...
case "${1:-toggle}" in
  ...
  *)              echo "Usage: $0 {start|menu|stop|toggle|status|status-json|waybar|is-active|settings}"; exit 2 ;;
esac
